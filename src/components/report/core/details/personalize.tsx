...
const Personalize = (props: customProps) => {
  ...
  const afterOpenChange = () => {
    let newData: any[] = [];
    props.columns.forEach((item, index: number) =>
      newData.push({
        key: index + 1,
        title: item.title ?? i18n.t(item.dataIndex),
        dataIndex: item.dataIndex,
        visible: item.visible ?? true,
        groupBy: item.groupBy ?? false,
      })
    );
    ...
    const viewItems = [
      {
        ...
        itemOptions: { ...form.getFieldsValue(), columns: table.data },
      },
    ];
    ...
    >
      <Form form={form}>
        <Form.Group>
          ...
          <Form.Item name="size">
            <SelectFixed dataSource="tableSize" />
          </Form.Item>
        </Form.Group>
        <Form.Group>
